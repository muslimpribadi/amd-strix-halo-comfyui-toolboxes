...
| Local ROCm 7.2 <br />[Dockerfile.rocm7.2](https://github.com/muslimpribadi/amd-strix-halo-comfyui-toolboxes/blob/main/Dockerfile.rocm7.2) | 18 GB | `PyTorch2.13 rocm7.2` | [`https://download.pytorch.org/whl/rocm7.2`](https://download.pytorch.org/whl/rocm7.2) |
...
This table compares the performance and size of different ComfyUI ROCm podman images. The best results for each metric are marked with 🏆.

| Metric / Workflow | ROCm 7.1.4<br>*(Original kyuz0)* | ROCm 7.2 | ROCm 7.1.3 | ROCm 7.14<br>(*🔴`pytorch 2.12`)* | ROCm 7.14<br>(*`pytorch 2.11`)* |
| :--- | :---: | :---: | :---: | :---: | :---: |
| `Flux1 Schnell 4 Steps` | **77 s** 🏆 | 82 s | 82 s | 88 s |  |
| `Hunyuan Video 1.5_720p_i2v 4 step lora` | **16 m 16 s** 🏆 | 17 m 4 s | 18 m 6 s | 18m 49 s |  |
| `Hunyuan Video 1.5_720p_t2v 4 step lora` | **16 m 18 s** 🏆 | 16 m 42 s | 17 m 16 s | 18m 28 s |  |
| `LTX2 I2V BF16` | 47 m 2 s | **17 m 47 s** 🏆 | 19 m 34 s | 19m 3 s |  |
| `LTX2 T2V BF16` | 38 m 0 s | **16 m 56 s** 🏆 | 17 m 31 s | 18m 55 s |  |
| `Qwen Image 2512 BF16 20 Steps` | **6 m 7 s** 🏆 | 7 m 11 s | 6 m 41 s | 7m 23 s |  |
| `Qwen Image 2512 BF16 4 Step LoRA` | 10 m 51 s | **1 m 53 s** 🏆 | 3 m 4 s | 5m 34 s |  |
| `Qwen Image Edit 2511 BF16 20 Steps` | **10 m 19 s** 🏆 | 11 m 23 s | 10 m 38 s | 11m 50 s |  |
| `Qwen Image Edit 2511 BF16 4 Step LoRA` | 19 m 14 s | **2 m 42 s** 🏆 | 3 m 28 s | 6m 24 s |  |
| `Wan2.2 I2V A14B 4steps lora rank64 Seko V1 FP16` | 31 m 39 s | 31 m 22 s 🏆 | 36 m 30 s | 36m 54 s |  |
| `Wan2.2 T2V A14B FP16 4steps lora rank64 Seko V2` | 29 m 42 s | 29 m 3 s 🏆 | 35 m 33 s | 36m 55 s |  |

### 💡 Interesting Facts & Observations

* **Size vs. Performance Sweet Spot**: The **Local ROCm 7.2 (Slim)** build effectively halves the image size (from 35.2 GB down to 18 GB) compared to the standard local build, without sacrificing generation speed—in fact, it even takes the crown for fastest LoRA performance. 
* **Tiny but Mighty**: The **Local ROCm 7.1 (Slim)** is remarkably small at just **8.18 GB** (an ~75% reduction from the standard build). While slightly slower on some heavy workflows like Wan2.2, it remains highly competitive across the board, making it perfect for storage-constrained setups.
